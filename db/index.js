...
async function getOpenReports() {
  try {
    ...
    const reportPromise = openReports.map(async (report) => {
      const { rows: commentsArray } = await client.query(
        `
      SELECT *
      FROM comments
      WHERE "reportId" IN ($1);
    ...
    const modifiedReports = await Promise.all(reportPromise);
